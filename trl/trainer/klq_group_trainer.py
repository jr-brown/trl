...
class RLOOStats(OnPolicyStats):
    def __init__(self, stats_shape: Tuple[int, int, int], device: torch.device):
        ...
        self.loss_stats = torch.zeros(stats_shape, device=device)
    ...
    def update(
        ...
        self.loss_stats[update_location] = loss
...
def micro_batch_updates(
    ...
    for micro_batch_start in range(
        ...
        with accelerator.accumulate(model):
            ...
            micro_batch_centred_scores = centred_scores[micro_batch_inds]
            ...
            new_ref_cumulative_log_ratio = new_ref_log_ratio.sum(1)
            ratio = torch.exp(
            ...
            losses = (
                config.kl_coef * new_ref_cumulative_log_ratio
                - micro_batch_centred_scores
            ) ** 2
            loss = losses.mean()
...
def rloo_batch_update(
    ...
    with torch.no_grad():
        ...
        # Follow RLOO code to centre scores to be mean zero. Note that we do not scale them to unit variance.
        scores_reshaped = scores.reshape(config.rloo_k, -1)
        centred_scores = scores_reshaped - scores_reshaped.mean(0, keepdim=True)
        grp_var = torch.var(centred_scores, dim=0, keepdim=False)  # [batch / rloo_k]
        centred_scores = centred_scores.flatten()
    ...
    with torch.no_grad():
        ...
        metrics_gathered_and_meaned = {
            ...
            "objective/traj/avg_grp_var": grp_var.mean(),
            #
